mfpcm <- function(x, centers, memberships, m=2, eta=2,  
  ...
  compd1 <- parse(text = paste0(".compdist(x[i,], vm, dmetric='", dmetric, "', p=", pw, ")"))
  if(!is.matrix(centers))
    centers <- matrix(nrow=k, ncol=p, eval(compv))
  ...
  for(start.idx in 1:nstart){
    ...
    cputime <- system.time(
      while((iter < iter.max) && (sum(abs(prevu - u)) > con.val)){
        ...
        for(j in 1:k)
          for(i in 1:n)
            w[i,j] <- exp(- (sum(d[i,j]/eval(compd1)) * k/n))
        for(j in 1:k){
          for(i in 1:n){
            if(min(d[i,])==0){
              u[i,] <- t[i,] <- rep(0, k)
              ...
              t[i,j] <- 1 / (sum(d[i,j]/d[,j])^(1/(eta-1)))
            }
          }
        }
        v <- t(u^m * w^m + t^eta * w^eta) %*% x / colSums((u^m * w^m) + (t^eta * w^eta))
      }
    )
    comp.time[start.idx] <- round(cputime[1],9)
    iter.num[start.idx] <- iter
    obj.func <- sum(d^(2*m) * u^m * w^m + d^(2*eta) * t^eta * w^eta)
