pca <- function(x, centers, memberships, m=2, eta=2, 
  ...
  dmetrics <- c("euclidean", "sqeuclidean", "manhattan", "minkowski", "sqchord", "chebyshev", "pearsonchi", 
               "neymanchi", "sqchi", "divergence", "addsymchi", "prosymchi", "clark", "canberra", "sorensen", 
               "lorentzian", "cosine")
  ...
  ssq <- 0
  for(i in 1:n)
    ssq <- ssq + (t(x[i,]-xcmean) %*% (x[i,]-xcmean))
  ...
  for(start.idx in 1:nstart){
        ...
        for(i in 1:n){
          for(j in 1:k){
            if(min(d[i,]) == 0){
               ...
               u[i,j] = ((1/d[i,j])^(1/(m-1)))/sum(((1/d[i,])^(1/(m-1))))
               t[i,j] = exp(-(eta*sqrt(k)*d[i,j])/beta)            
            } 
          }
        }
    ...
    #obj.func <- sum(exp(-(eta*sqrt(k)*d/beta)^eta))
    obj.func <- sum(t^eta*d) + (beta/(eta^2*sqrt(k))) * (sum(t^eta * log(t^eta) - t^eta))
