gkpfcm <- function(x, centers, memberships, m=2, eta=2, K=1, omega, a, b, 
  ...
  if(!is.numeric(a) || !is.numeric(b))
    stop("The coefficients 'a' and 'b' should be a number greater than 0")
  if((a < 0) || (b < 0))
    stop("The coefficients 'a' and 'b' should be a number greater than 0")
  ...
  for(start.idx in 1:nstart){
    ...
    if(omega.recomp)
      omega <- comp.omega(d=d, u=memberships, m=eta, K=K)
    v0 <- v <- centers
    u <- memberships   
    ...
    cputime <- system.time(
      while((iter < iter.max) && (sum(abs(prevu - u)) > con.val)){
        ...
        v <- t(a*u^m + b*t^eta) %*% x / colSums(a*u^m + b*t^eta)
        for(j in 1:k){
          ...
          alpha <- sum(u[,j]) / n
          for(i in 1:n)
            d[i,j] <- (2*pi)^(n/2) * sqrt(det(aj)) * t(x[i,]-v[j,]) %*% (MASS::ginv(aj, tol = 0)) %*% (x[i,]-v[j,])/alpha
        }
        for(i in 1:n){
          u[i,] <- 1/(((d[i,])^(1/(m-1))) * sum((1/(d[i,]))^(1/(m-1))))
          t[i,] <- 1/(1 + (b*(d[i,]/omega[j]))^(1/(eta-1)))
        }
    ...
    obj.func <- 0
    for(j in 1:k)
      obj.func <- obj.func + sum(d[,j]*(a*(u[,j]^m) + b*(t[,j]^eta))) + sum(omega[j]) * sum((1-(t[,j])^eta))
