upfc <- function(x, centers, memberships, m=2, eta=2, a, b,
  ...
  compd1 <- parse(text = paste0(".compdist(x[i,], v[l,], dmetric='", dmetric, "', p=", pw, ")"))
  if(!is.matrix(centers))
  ...
  for(start.idx in 1:nstart){
        ...
        for(i in 1:n){
          for(j in 1:k){
            if(min(d[i,]) == 0){
              ...
              t[i,j] <- exp(-((b*eta*sqrt(k)*d[i,j])/beta))
            }
          }
        }
    ...
    for(i in 1:n){
      for(j in 1:k){
        tot1 <- tot1 + (a*u[i,j]^m + b*t[i,j]^eta) * (d[i,j])
        if(t[i,j] != 0)
          tot2 <- tot2 + (beta / eta^2 * sqrt(k)) * (t[i,j]^eta * log(t[i,j]^eta,exp(1)) - t[i,j]^eta)
      }
    } 
    obj.func = tot1 + tot2
