gg <- function(x, centers, memberships, m=2, ggversion="simple", 
      dmetric="sqeuclidean", pw=2, alginitv="kmpp", alginitu="imembrand", 
      nstart=1, iter.max=1e03, con.val=1e-09, 
  ...
  if(!missing(memberships)){
    ...
    if(!is(memberships, "matrix"))
      stop("The initial matrix of membership degrees is not a numeric matrix")
  }else{
  ...
  if(!is.element(ggversion, c("original", "simple")))
    stop("Value of argument 'ggversion' should be either 'original' or 'simple'")
  if(!is.logical(stand))
  ...
  for(start.idx in 1:nstart){
    ...
    cputime <- system.time(
      while((iter < iter.max) && (sum(abs(prevu - u)) > con.val)){
        ...
        for(j in 1:k){
          f[,,j] = as.matrix(nrow=p, ncol=p, 0); aj <- f[,,j]
          alpha <- numeric(k)
          for(i in 1:n)
            aj <- aj + (u[i,j]^m) * ((x[i,]-v[j,]) %*% t((x[i,]-v[j,])))
          aj <- aj/sum(u[,j]^m); f[,,j] <- aj
          alpha[j] <- sum(u[,j]^m)/sum(u^m)
          for (i in 1:n){
            if(ggversion=="original")
              d[i,j] <- (((2*pi)^(p/2) * sqrt(det(aj)))/ alpha[j]) * exp((1/2) * 
                        t(x[i,]-v[j,]) %*% (MASS::ginv(aj, tol = 0)) %*% (x[i,]-v[j,]))
            else if(ggversion=="simple")
              d[i,j] <- (sqrt(det(aj))/ alpha[j]) * exp((1/2) * 
                        t(x[i,]-v[j,]) %*% (MASS::ginv(aj, tol = 0)) %*% (x[i,]-v[j,]))  
          }
        }
        for(i in 1:n)
