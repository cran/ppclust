pcmr <- function(x, centers, memberships, eta=2, K=1, omega, gamma=15,
  ...
  compd1 <- parse(text = paste0(".compdist(v[j,], v[l,], dmetric='", dmetric, "', p=", pw, ")"))
  if(!is.matrix(centers))
  ...
  if(!is.numeric(gamma))
    stop("The argument 'gamma' must be a number")
  if(gamma <= 1)
    stop("The argument 'gamma' must be greater than 1")
  ...
  for(start.idx in 1:nstart){
    ...
    iter <- 0
    cputime <- system.time(
      while((iter < iter.max) && (sum(abs(prevt - t)) > con.val)){
        iter <- iter + 1
        prevt <- t
        for(i in 1:n)
          for(j in 1:k)
        ...
        for(j in 1:k){
          ...
          for(i in 1:n){
            total1 <- total1 + t[i,j] %*% x[i,]
            total2 <- total2 + t[i,j]
          }
          for(l in 1:k){
            if(j != l){
              dv <- 1 / c(t(v[j]-v[l])*(v[j]-v[l]))^2
              total3 <- total3 + v[j,]*dv
              total4 <- total4 + dv
            }
          }
          v[j,] = c(total1 - gamma * total3)/(total2 - gamma * total4)
        }
      }
    )
    ...
    for(j in 1:k){
      ...
      for(i in 1:n){
        total1 <- total1 +  t[i,j]^eta * d[i,j] 
        total2 <- total2 + (1-(t[i,j]^eta))
      }
      for(l in 1:k)
        if(j != l)
          total3 <- total3 + c(1/eval(compd1))
    }
    obj.func <- total1 + sum(omega) * total2 + gamma * total3
